...
if (!customElements.get('glass-gas-card-editor')) {
  customElements.define('glass-gas-card-editor', GasBalanceCardEditor);
}

class GasBalanceCard extends LitElement {
  static getConfigElement() {
    return document.createElement("glass-gas-card-editor");
  }
  ...
  constructor() {
    ...
    this.activeNav = '';
    this.monthData = null;
    this.colorNum = '#8ce0b8';
    this.colorCost = '#ffb07c';
  ...
  static get styles() {
    return css`
      ...
      .balance-icon {
        ...
        border: 0;
        box-shadow: none;
        filter: drop-shadow(0 8px 18px rgba(38, 141, 230, 0.18));
      }
      ...
      .balance-info {
        ...
        box-shadow: inset 0 1px 0 rgba(255,255,255,0.14), 0 8px 22px rgba(3,9,20,0.16);
      ...
      .action-button {
        ...
        border: 1px solid rgba(255,255,255,0.18);
        box-shadow: inset 0 1px 0 rgba(255,255,255,0.14), 0 8px 20px rgba(3,8,18,0.16);
        backdrop-filter: blur(12px) saturate(118%);
        -webkit-backdrop-filter: blur(12px) saturate(118%);
      }
      
      .action-button.active {
        background: linear-gradient(180deg, rgba(100,233,246,0.4), rgba(42,190,214,0.22)) !important;
        color: #dffcff;
        font-weight: 700;
        border-color: rgba(112,233,245,0.48);
        box-shadow: inset 0 1px 0 rgba(255,255,255,0.22), 0 12px 28px rgba(18,133,152,0.24);
      }
      ...
      .action-button.active:hover {
        background: linear-gradient(180deg, rgba(112,236,248,0.48), rgba(52,196,220,0.24)) !important;
      }
      ...
      .ladder-section {
        ...
        box-shadow: inset 0 1px 0 rgba(255,255,255,0.12), 0 10px 26px rgba(4,10,22,0.16);
      ...
      .usage-section {
        ...
        background:
          ...
          rgba(255,255,255,0.07) !important;
        border: 1px solid rgba(255,255,255,0.16);
        box-shadow: inset 0 1px 0 rgba(255,255,255,0.1), 0 8px 20px rgba(3,8,18,0.12);
        backdrop-filter: blur(12px) saturate(118%);
        -webkit-backdrop-filter: blur(12px) saturate(118%);
      }
     ...
     *          */
      ...
      .calendar-grid {
        ...
        padding: 12px 8px;
        margin-top: 5px;
        box-shadow: inset 0 1px 0 rgba(255,255,255,0.14), 0 14px 36px rgba(3,8,18,0.16);
  ...
  _renderDayChart() {
    ...
    if (this._dayChartRenderTimeout) clearTimeout(this._dayChartRenderTimeout);
    if (this._dayChartUpdateTimeout) clearTimeout(this._dayChartUpdateTimeout);
    if (this._chartResizeObserver) {
      this._chartResizeObserver.disconnect();
      this._chartResizeObserver = null;
    }
    ...
    this._dayChartRenderTimeout = setTimeout(() => {
      ...
      if (containerWidth > 0) {
        ...
        const markerOptions = this._getGasDayMaxMarker(data);
  ...
  _renderMonthChart() {
    ...
    if (this._monthChartRenderTimeout) clearTimeout(this._monthChartRenderTimeout);
    if (this._monthChartUpdateTimeout) clearTimeout(this._monthChartUpdateTimeout);
    if (this._chartResizeObserver) {
      this._chartResizeObserver.disconnect();
      this._chartResizeObserver = null;
    }
    ...
    this._monthChartRenderTimeout = setTimeout(() => {
      ...
      if (containerWidth > 0) {
        ...
        const markerOptions = this._getGasMonthMaxMarker(data);
  ...
  _getGasDayMaxMarker(data) {
    const values = data.electricity.map(item => item.y);
    const maxValue = values.length ? Math.max(...values) : 0;
    const maxPoint = data.electricity.find(item => item.y === maxValue);
    if (!maxPoint || maxValue <= 0) return null;
    const date = new Date(maxPoint.x);
    return {
      categoryIndex: date.getDate() - 1,
      categoryValue: date.getDate(),
      label: `${maxValue.toFixed(2)}m³`,
      color: this.colorNum
    };
  }

  _getGasMonthMaxMarker(data) {
    const values = data.electricity.map(item => item.y);
    const maxValue = values.length ? Math.max(...values) : 0;
    const maxPoint = data.electricity.find(item => item.y === maxValue);
    if (!maxPoint || maxValue <= 0) return null;
    const month = new Date(maxPoint.x).getMonth() + 1;
    return {
  ...
  _getChartDayConfig(data) {
    ...
    const categories = Array.from(new Set([
      ...data.electricity.map(item => dayLabel(item.x)),
      ...data.cost.map(item => dayLabel(item.x))
    ]));
    const toCategorySeries = (arr) => {
      const valueMap = new Map(arr.map(item => [dayLabel(item.x), item.y]));
    ...
    return {
      series: [
        {
          name: `日用气量`,
          data: toCategorySeries(data.electricity),
          type: 'column',
          zIndex: 0
        },
        {
          name: `日用气金额`,
          data: toCategorySeries(data.cost),
          type: 'line',
          color: colorCost,
          zIndex: 1
        }
      ...
      xaxis: {
        ...
        labels: {
          ...
          formatter: function(val) {
            const day = Number(String(val).split('-').pop());
      ...
      tooltip: {
        ...
        custom: function({ series, seriesIndex, dataPointIndex, w }) {
          const formattedDate = categories[dataPointIndex] || '';
          const seriesInfo = [
  ...
  _getChartMonthConfig(data) {
    ...
    const categories = Array.from(new Set([
      ...data.lastelectricity.map(item => monthNumber(item.x)),
      ...data.electricity.map(item => monthNumber(item.x)),
    ...
    return {
      series: [
        {
          name: `上年气量`,
          data: toCategorySeries(data.lastelectricity),
          type: 'column',
          zIndex: 0,
          color: "rgba(169, 239, 205, 0.30)"
        },
        {
          name: `本年气量`,
          data: toCategorySeries(data.electricity),
        ...
        {
          name: `上年金额`,
          data: toCategorySeries(data.lastcost),
          type: 'line',
          color: "rgba(255, 208, 173, 0.34)",
          zIndex: 2
        },
        {
          name: `本年金额`,
      ...
      colors: ['rgba(169, 239, 205, 0.30)', colorNum, 'rgba(255, 208, 173, 0.34)', colorCost],
      plotOptions: {
        bar: {
          horizontal: false,
          borderRadius: 0,
      ...
      tooltip: {
        ...
        custom: function({ series, seriesIndex, dataPointIndex, w }) {
          ...
          const seriesInfo = [
            { name: '上年气量', unit: 'm³', color: 'rgba(169, 239, 205, 0.72)' },
            { name: '本年气量', unit: 'm³', color: this.colorNum },
            { name: '上年金额', label: '上年气费', unit: '元', color: 'rgba(255, 208, 173, 0.72)' },
            { name: '本年金额', label: '本年气费', unit: '元', color: this.colorCost }
          ];
          let tooltipHTML = `
            <div style="background: ${BgColor};color: ${Color};padding: 8px;border-radius: 4px;border: 1px solid ${Color};">
              <div style="font-weight: bold; font-size: 12px;color: ${Color};  border-bottom: 1px dashed #999;">
 ...
 /*渲染日度用气条形图*/
  ...
  renderCalendar() {
    ...
    for (let i = 1; i <= daysInMonth; i++) {
      ...
      const dayContent = html`
        <div>${i}</div>
        ${dayData ? html`
          <div class="electricity-num" style="color: #a9efcd">${dayData.dayGasNum}m³</div>
          <div class="electricity-cost" style="color: #ffd0ad">${dayData.dayGasCost}元</div>
        ` : ''}
      `;
    ...
    const bottomRow = html`
      ...
      <div class="cell summary-info" style="grid-area: id99;">
        ${this.monthData ? html`
          <div><span style="color: #a9efcd">月气量: ${this.monthData.monthGasNum}m³</span></div>
          <div><span style="color: #ffd0ad">月气费: ${this.monthData.monthGasCost}元</span></div>
        ` : html`<div></div>`}
      </div>
    `;
    return html`
  ...
  renderMain() {
    ...
    const svgpath = theme === 'on' ? '/glass_energy_cards-local/icon/gas-on.svg' : '/glass_energy_cards-local/icon/gas-off.svg';
...
if (!customElements.get('glass-gas-card')) {
  customElements.define('glass-gas-card', GasBalanceCard);
}
